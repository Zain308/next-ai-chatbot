...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      const suggestionPrompt = `Based on the query "${query.trim()}", provide exactly 3 short follow-up questions (each under 8 words) that would be relevant to continue this conversation. Return only the questions, one per line, without numbers or bullets.`
